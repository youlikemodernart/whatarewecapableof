...
function adminCookie() {
  return signedCookieFor('noah@whatarewecapableof.com');
}
...
result = await call(session, { url: '/api/session', cookie: adminCookie() });
assert(result.status === 200 && result.data.user?.email === 'noah@whatarewecapableof.com', 'admin smoke session should validate');

result = await call(responses, { url: '/api/admin/responses', cookie: adminCookie() });
...
result = await call(exportMarkdown, { url: `/api/admin/export?id=${encodeURIComponent(responseId)}`, cookie: adminCookie() });
...
console.log(JSON.stringify({
  ...
  checks: ['health', 'metadata-hides-questions', 'passcode', 'opaque-draft-cookie', 'duplicate-submit-blocked', 'admin-auth', 'admin-allowlist', 'markdown-export'],
}, null, 2));
