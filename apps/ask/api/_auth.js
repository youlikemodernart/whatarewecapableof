...
const SESSION_COOKIE = 'wawco_ask_session';
const OAUTH_STATE_COOKIE = 'wawco_ask_oauth_state';
const DRAFT_COOKIE = 'wawco_ask_draft';
const CSRF_COOKIE = 'wawco_ask_csrf';
...
function getBaseUrl(req) {
  const configured = env('ASK_BASE_URL');
...
function authConfig() {
  const clientId = env('ASK_GOOGLE_CLIENT_ID');
  const clientSecret = env('ASK_GOOGLE_CLIENT_SECRET');
  const sessionSecret = env('ASK_SESSION_SECRET');
  const allowedDomain = env('ASK_ALLOWED_DOMAIN', DEFAULT_ALLOWED_DOMAIN).toLowerCase();
  const allowedEmailSource = env('ASK_ADMIN_EMAILS') || env('ASK_ALLOWED_EMAILS') || 'noah@whatarewecapableof.com';
  const allowedEmails = allowedEmailSource
    .split(',')
...
function storageConfig() {
  const explicitMode = env('ASK_STORAGE_MODE');
  const databaseConfigured = Boolean(env('ASK_POSTGRES_URL') || env('POSTGRES_URL') || env('DATABASE_URL'));
  const mode = explicitMode || (databaseConfigured ? 'postgres' : 'unconfigured');
  return {
    configured: mode === 'postgres' ? databaseConfigured : mode === 'memory',
    mode,
  };
}
...
function signingSecret() {
  return authConfig().sessionSecret || env('ASK_DEV_SESSION_SECRET');
}

function sign(value, secret = signingSecret()) {
  if (!secret) throw new Error('Missing ASK_SESSION_SECRET');
  return crypto.createHmac('sha256', secret).update(String(value)).digest('hex');
}

function cookieAttrs(req, maxAgeSeconds, sameSite = 'Lax') {
...
function createSessionCookie(req, user) {
  ...
  const value = `${payload}.${sign(payload)}`;
  return [
    `${SESSION_COOKIE}=${encodeURIComponent(value)}; ${cookieAttrs(req, 14 * 24 * 60 * 60)}`,
    `${CSRF_COOKIE}=${encodeURIComponent(csrf)}; Path=/; SameSite=Lax; ${req.headers['x-forwarded-proto'] === 'https' || env('VERCEL') === '1' ? 'Secure; ' : ''}Max-Age=${14 * 24 * 60 * 60}`,
  ];
}
...
function getSession(req) {
  const config = authConfig();
  if (!config.sessionSecret && !env('ASK_DEV_SESSION_SECRET')) return null;
  ...
  const payload = `v1.${parts[1]}.${parts[2]}`;
  if (!safeEqual(parts[3], sign(payload, config.sessionSecret || env('ASK_DEV_SESSION_SECRET')))) return null;
...
function requireCsrf(req) {
  ...
  const header = String(req.headers['x-ask-csrf'] || '');
  return Boolean(user.csrf && cookies[CSRF_COOKIE] && header && safeEqual(user.csrf, cookies[CSRF_COOKIE]) && safeEqual(header, user.csrf));
}

function createDraftCookie(req, draft) {
  const expiresAt = Math.floor(Date.now() / 1000) + 7 * 24 * 60 * 60;
  const body = Buffer.from(JSON.stringify({
    responseId: draft.responseId,
    deckId: draft.deckId,
    deckVersionId: draft.deckVersionId,
  })).toString('base64url');
  const payload = `v1.${expiresAt}.${body}`;
  const value = `${payload}.${sign(payload)}`;
  return `${DRAFT_COOKIE}=${encodeURIComponent(value)}; ${cookieAttrs(req, 7 * 24 * 60 * 60)}`;
}
...
function getDraft(req) {
  ...
  const value = parseCookies(req.headers.cookie || '')[DRAFT_COOKIE];
  if (!value) return null;
  const parts = value.split('.');
  if (parts.length !== 4 || parts[0] !== 'v1') return null;
  const expiresAt = Number(parts[1]);
